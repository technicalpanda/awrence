...
RSpec.describe "Hash" do
  describe "#to_camelback_keys" do
    context "when the hash contains simple keys with snake_case" do
      subject(:camelized) { hash.to_camelback_keys }
    ...
    context "with a key that is an acronym" do
      ...
      it "camelizes the acronym" do
        expect({ "user_id" => "1" }.to_camelback_keys.keys.first).to eq("userID")
      end

      it "respects camelback boundaries" do
        expect({ "id" => "1" }.to_camelback_keys.keys.first).to eq("id")
      end

      it "matches on word boundaries" do
        expect({ "idee" => "1" }.to_camelback_keys.keys.first).to eq("idee")
        expect({ "some_idee" => "1" }.to_camelback_keys.keys.first).to eq("someIdee")
      end
    end

    context "with keys that have spaces in them" do
      it "doesn't get camelized" do
        expect({ "With Spaces" => "FooBar" }.to_camelback_keys.keys.first).to eq("With Spaces")
      end
    end

    context "with keys that aren't strings or symbols" do
      it "doesn't get camelized" do
        expect({ 1 => "FooBar" }.to_camelback_keys.keys.first).to eq(1)
      end
    end
  end
end
